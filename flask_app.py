    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if not inspector.has_table('financial_data'):
                logger.info("Criando tabela financial_data...")
                conn.execute(text("""
                    ...
                    """))
                logger.info("[OK] Tabela financial_data criada com sucesso.")
            else:
                logger.info("Tabela financial_data já existe.")
...
def get_companies_list(engine, ticker_mapping_df):
    ...
    try:
        with engine.connect() as connection:
            ...
            required_columns = {'cd_cvm', 'denom_cia'}  
            ...
            # Executa a query
            query = text('SELECT DISTINCT cd_cvm, denom_cia AS nome_empresa FROM financial_data ORDER BY denom_cia')
...
try:
    ...
    if is_db_connected(db_engine):
        ...
        if ensure_valuation_table_exists(db_engine):
            # Executar worker apenas se necessário
